...
public class GeneticAlgorithms {
	...
	public static final int initPopulationSize = 300;
	...
	public static ArrayList<Arrangement> PMX(Arrangement arr1, Arrangement arr2) {
		...
		/*////////////FOR_DEBUG//////////////////////////////
		boolean arr1Wrong = false;
		boolean arr2Wrong = false;
		
		int counter = 0;
		for (int l = 0; l < arr1.getLength(); l++) {
			if (arr1.getList().indexOf(arr1.getCamper(l)) != arr1.getList().lastIndexOf(arr1.getCamper(l))) {
				counter++;
			}
		}
		if (counter > 0) {
		...
		counter = 0;
		for (int l = 0; l < arr2.getLength(); l++) {
			if (arr2.getList().indexOf(arr2.getCamper(l)) != arr2.getList().lastIndexOf(arr2.getCamper(l))) {
				counter++;
			}
		}
		if (counter > 0) {
		...
		/////FOR_SECOND_CHECK_OF_DEBUG/////
		counter = 0;
		for (int l = 0; l < offspring1.size(); l++) {
			if (offspring1.get(l) != null && offspring1.indexOf(offspring1.get(l)) != offspring1.lastIndexOf(offspring1.get(l))) {
				counter++;
			}
		}
		if (counter > 0 && !arr1Wrong && !arr2Wrong) {
			System.out.println("Cross 1: " + crossPoint1 + " Cross 2: " + crossPoint2);
			System.out.println("\nParent 1");
			for (Camper c : arr1.getList()) {
				System.out.println(c.getName());
			}
			System.out.println("\nParent 2");
			for (Camper c : arr2.getList()) {
				System.out.println(c.getName());
			}
			System.out.println("\n Map 1");
			for (Camper c : map1) {
		...
		counter = 0;
		for (int l = 0; l < offspring2.size(); l++) {
			if (offspring2.get(l) != null && offspring2.indexOf(offspring2.get(l)) != offspring2.lastIndexOf(offspring2.get(l))) {
				counter++;
			}
		}
		if (counter > 0 && !arr1Wrong && !arr2Wrong) {
			System.out.println("Cross 1: " + crossPoint1 + " Cross 2: " + crossPoint2);
			for(Camper c : offspring2) {
				if (c != null)
					System.out.println(c.getName());
			}
			System.out.println();
